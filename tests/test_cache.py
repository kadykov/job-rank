...
EXPECTED_EXPLANATION_CACHE_KEY = hashlib.sha256(
    EXPECTED_EXPLANATION_HASH_INPUT.encode("utf-8")
).hexdigest()
...
def test_get_cache_key_sensitivity():
    ...
    key_base = get_cache_key(
        MOCK_JD_PATH, MOCK_JD_TEXT, MOCK_CONFIG, MOCK_SYSTEM_PROMPT
    )

    # Change JD text
    key_diff_text = get_cache_key(
        MOCK_JD_PATH, "Different JD text.", MOCK_CONFIG, MOCK_SYSTEM_PROMPT
    )
    ...
    key_diff_llm = get_cache_key(
        MOCK_JD_PATH, MOCK_JD_TEXT, config_diff_llm, MOCK_SYSTEM_PROMPT
    )
    ...
    key_diff_emb = get_cache_key(
        MOCK_JD_PATH, MOCK_JD_TEXT, config_diff_emb, MOCK_SYSTEM_PROMPT
    )
    assert key_base != key_diff_emb

    # Change System Prompt
    key_diff_prompt = get_cache_key(
        MOCK_JD_PATH, MOCK_JD_TEXT, MOCK_CONFIG, "Different prompt."
    )
...
def test_save_to_cache_success(mock_open_file, mock_np_save, mock_mkdir):
    ...
    mock_open_file.assert_called_once_with(
        EXPECTED_IDEAL_CV_PATH, "w", encoding="utf-8"
    )
...
def test_get_explanation_cache_path():
    ...
    assert (
        exp_path
        == expected_subdir / f"{EXPECTED_EXPLANATION_CACHE_KEY}_explanation.txt"
    )
...
def test_save_explanation_to_cache_success(mock_open_file, mock_mkdir):
    ...
    save_explanation_to_cache(
        cache_dir, EXPECTED_EXPLANATION_CACHE_KEY, MOCK_EXPLANATION_TEXT
    )
